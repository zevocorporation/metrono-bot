...
const orderScene= new WizardScene(
    ...
    ctx=>
    {
        if(ctx.message.text!="Today" && ctx.message.text!="Tomorrow" && ctx.message.text!="Home")
        {
            const keyboard=new Keyboard();
            keyboard.add("Home");
    ...
    ctx=>{
        ...
        if(ctx.message.text!="Breakfast" && ctx.message.text!="Lunch" && ctx.message.text!="Dinner" && ctx.message.text!="Home")
        {
            const keyboard=new Keyboard();
            keyboard.add("Home");
            ctx.reply("Please Choose from given options!",keyboard.draw())
    ...
    async ctx=>{
        ...
        if(ctx.message.text!="1" && ctx.message.text!="2" && ctx.message.text!="3" && ctx.message.text!="4" && ctx.message.text!="5" && ctx.message.text!="Home")
        {
            const keyboard=new Keyboard();
            keyboard.add('Wallet','Menu').add('Subscribe Plans','Order Meals','Order Addons').add('My Plans','My Account');
            ctx.reply("Please Choose from given options!",keyboard.draw())
            ...
            var promise = new Promise(function(resolve, reject) { 
                ...
                request.post('https://test.instamojo.com/api/1.1/payment-requests/', { form: payload, headers: headers }, function (error, response, body) {
                    if (!error && response.statusCode == 201) {
                        ...
                        ctx.reply(`Your order amount is${amount}`,Markup.removeKeyboard().extra())
